...
public class NodoM {
    ...
    public int posData(int x) { // Devuelve la posicion de x si existe en el nodo.
        for (int i = 0; i < data.length; i++) {
            if (usada[i] && data[i] == x) {
                return i + 1;
            }
    ...
    private boolean indexValido(int i, int n, String Metodo) {
        ...
        if (!b)
            System.err
                    .println("NodoM." + Metodo + ": " + i + " está fuera de rango. Los índices van de 1 a " + n + ".");
